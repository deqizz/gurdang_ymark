...
function post_data(){
  ...
  if(validator.form()){
    ...
    // $.post('url', data);
    var api = base_url+"data/catatan_bk/post";
...
$(document).on('click','.delete',function(){
  ...
  bootbox.confirm("Apakah anda yakin akan menghapus data catatan barang masuk"+$(tr).find('td').eq(1).html()+" - "+$(tr).find('td').eq(2).html()+"? ", function(result){
    if (result) {
      $.post( base_url+"data/catatan_bk/delete", {id : idx}).done(function( data ) {
        if (data == '0'){
...
$(document).on('click','.update',function(){
  ...
  var tr = $(this).closest('tr');
  $('#active').show();
  $.post( base_url+"data/catatan_bk/get_by_id", {idx: idx}).done(function( data1 ) {
    var json = $.parseJSON(data1);
    console.log(json)
    $('[name="kode_catatan_bk"]').val(json.kode_catatan_bk);
    $('[name="kode_brg"]').val(json.kode_brg);
    $('[name="jml_bk"]').val(json.jml_bk);
    $('[name="tgl"]').val(json.tgl);
    $('[name="nip_bag_pemasaran"]').val(json.nip_bag_pemasaran);
  });
});
